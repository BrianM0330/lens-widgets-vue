...
export default defineComponent({
  ...
  computed: {
    backgroundColor () {
      let { backgroundColor } = getColors(this.theme) as any

      return this.iconBackgroundColor ? this.iconBackgroundColor : backgroundColor
    },
    foregroundColor () {

      let { foregroundColor } = getColors(this.theme) as any

      return this.iconForegroundColor ? this.iconForegroundColor : foregroundColor
    }
  },
  mounted () {
    const { width, height } = dimensionsMap[this.size]
    this.width = width
    this.height = height
  },
  methods: {
    getColors(theme: Theme) {
      let foregroundColor = backgroundColorMap[theme]
      let backgroundColor = foregroundColorMap[theme]
      return {
